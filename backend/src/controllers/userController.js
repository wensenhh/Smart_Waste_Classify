...
const register = async (ctx) => {
  const { phone, password, name, phoneCode } = ctx.request.body;
  const t = ctx.i18n ? ctx.i18n.t : (key) => key;
  console.log('注册请求体参数:', ctx.request.body);
  try {
    // 验证输入
    if (!phone || !password || !phoneCode) {
    ...
    // 获取用户IP地址
    const userIP = ipUtils.getUserIP(ctx);
    
    // 根据区号获取国家信息
    const country = ipUtils.getCountryByPhoneCode(phoneCode);
    
    // 创建用户 - 添加phone_code, country, registration_ip字段
    const result = await db.insert(
      'INSERT INTO users (id, phone, password, name, phone_code, country, registration_ip) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [userId, phone, hashedPassword, name || '用户' + phone, phoneCode, country, userIP]
    );
